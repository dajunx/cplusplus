...
struct employee {
  ...
  //employee(int id_,std::string name_,int age_):id(id_),name(name_),age(age_), grade(0){}
...
boost::shared_ptr<employee>,
      ...
      tag<strgrade>,
      key_from_key <
      BOOST_MULTI_INDEX_MEMBER(sub_ee, int, grade),
...
void print(employee_set& es)
{
  ...
  for (; age_map_begin != age_.end(); age_map_begin++)
    std::cout<<"id:"<<(*age_map_begin)->id
             ...
             <<", index:" << (*age_map_begin)->index
             <<", grade"<<(*age_map_begin)->grade.grade
             <<std::endl;
}
...
int main()
{
  ...
  es.insert(boost::make_shared<employee>(employee(0, "1", 55, 0, str_grade(2))));
  es.insert(boost::make_shared<employee>(employee(1, "1", 20, 1, str_grade(5))));
  es.insert(boost::make_shared<employee>(employee(2, "2", 40, 2, str_grade(3))));  
  es.insert(boost::make_shared<employee>(employee(2, "1", 10, 4, str_grade(6))));
  es.insert(boost::make_shared<employee>(employee(2, "2", 35, 5, str_grade(4))));  
  es.insert(boost::make_shared<employee>(employee(6, "1", 80, 6, str_grade(1))));
  ...
  boost::shared_ptr<employee> ptr_tmp = boost::make_shared<employee>(
    employee((*age_map_per)->id, (*age_map_per)->name, (*age_map_per)->age, (*age_map_per)->index, (*age_map_per)->grade)
    );
  ptr_tmp->age = 30;
