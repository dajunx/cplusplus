...
bool ReadDataFromFileLBLIntoString(const std::string& filename, std::vector<std::string>& file_data)
{
  ...
  if (fin.is_open() == true) {
    ...
    std::cout<<"------- 名词.txt 或 动词.txt 文件 未找到，请创建--------"<<std::endl;
  }
...
int main(int argc,char *argv[]) 
{
  ...
  std::cout<<std::endl;
  std::cout<<"请在程序当前目录放置 名词.txt、动词.txt 两个待处理文件"<<std::endl;
  std::cout<<std::endl;
  std::cout<<"------------------处理中...---------------------"<<std::endl;
  if (checkTwoFileExist() == false) {
    std::cout<<"检测到当前目录没有 名词.txt、动词.txt 两个待处理文件"<<std::endl;
    Sleep(3000);
  }
  ...
  szlog<<"输出文件名: "<<outPutFileName
    ...
    <<", 名词输入量: "<<input_file2_count
    <<", 合成量: "<<merge_count<<std::endl;
  writelog(szlog.str());
