/*
  测试 template 编程容易出现的一个问题。尚不能描述错处的原因，只知道当template和typedef有冲突的case。
  TODO 待研究
*/
#include <iostream>
...
public:
  ...
  //ps:若把上述两行逻辑修改为如下实现，则child2 调用 fun1函数能够正常调用show函数
  // template <typename child1> void fun1(boost::shared_ptr<base> const &ptr_base);
};
...
int main() {
  ...
  // 在当前程序编码环境下，如下调用fun1函数 会造成 ptr_ch为NULL的情况，原因待探究
