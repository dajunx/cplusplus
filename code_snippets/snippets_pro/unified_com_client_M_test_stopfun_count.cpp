...
/*
1、调用 stop 静态函数。测试来看，io_service的stop函数调用之后，会执行完毕 post到io_service上的handler，stop调用之后post的handler不会执行。
可能的原因是 完成端口的调度室FIFO，stop函数调用会 PostQueuedCompletionStatus 投递一个关闭run死循环的投递，当run读取到该函数的时候，会走退出流程。
2、若io_service 在stop之后，应用post请求，应用计数以及io_service 计数不会清零.
3、post出去的投递，若使用weak_ptr 则增加对象的weak_count，对应handler调用的时候查看use_count是否为0，
  此事对象是否已经析构，而shared_from_this 会增加对应的use_count，handler调用的时候对象肯定未释放。
4、禁用 IOCP ，使用宏 BOOST_ASIO_DISABLE_IOCP
*/
#include <iostream>
#include <vector>
    ...
    if (stop) {
      ...
      while (ptr_thread) {
        if (ptr_thread->try_join_for(boost::chrono::milliseconds(100))) {
          ...
          ptr_io_service->stop(); // 先stop 后在post，若post是shared_from_this方式，io_service会管着 计数不会释放
