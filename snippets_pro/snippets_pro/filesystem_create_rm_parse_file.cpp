...
void create_file(filesystem::path& path_temp)
{
  try
  {
    if (filesystem::exists(path_temp) && !filesystem::is_regular_file(path_temp))
    {
...
void parse(filesystem::fstream& fs, std::map<std::string, std::vector<data> >& dd)
{
  char content[50];
  std::string title;
...
void load_content_from_file(filesystem::path& path_file, std::map<std::string, std::vector<data> >& dd)
{
  try
  {
    if(filesystem::exists(path_file) && filesystem::is_regular_file(path_file))
    {
      filesystem::fstream fs;
...
void save_content()
{
  try
  {
    ...
    filesystem::fstream fs;
    fs.open("e:\\temp\\test1.txt", std::fstream::in | std::fstream::out | std::fstream::app);
    fs<<"["<<content<<"]"<<std::endl;
