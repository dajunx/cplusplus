...
void fun1(const std::string& db)
{
  try
  {
    ...
    BOOST_FOREACH(boost::property_tree::ptree::value_type &value, ptree.get_child("keywords"))
    {
      ...
      boost::uint32_t replact = child.get<boost::uint32_t>("replace") ? true:false;
    }

    BOOST_FOREACH(boost::property_tree::ptree::value_type &value, ptree.get_child("keywords5"))
    {
      boost::property_tree::ptree& child = value.second;
...
void printTree(const std::string& db) 
{
  ...
  try
  {
    for (boost::property_tree::ptree::iterator pos = pt.begin(); pos != pt.end();++pos) {
      ...
      BOOST_FOREACH(boost::property_tree::ptree::value_type &value, child.get_child(tmp.str(), empty_child))
      {
...
void fun2(const std::string& db)
{
  try
  {
    ...
    BOOST_FOREACH(boost::property_tree::ptree::value_type &value, ptree.get_child("0"))
    {
      boost::property_tree::ptree& child = value.second;
      boost::property_tree::write_json("filter_new1.txt", child);
    }

    BOOST_FOREACH(boost::property_tree::ptree::value_type &value, ptree.get_child("1"))
    {
      boost::property_tree::ptree& child = value.second;
      boost::property_tree::write_json("filter_new2.txt", child);
    }

    BOOST_FOREACH(boost::property_tree::ptree::value_type &value, ptree.get_child("2"))
    {
      boost::property_tree::ptree& child = value.second;
...
void fun3(const std::string& db)
{
  try
  {
    ...
    for (int i=0;i<static_cast<boost::uint32_t>(-1) && nCount <size; ++i)
    {
      ...
      BOOST_FOREACH(boost::property_tree::ptree::value_type &value, ptree.get_child(child_name, empty_tree))
      {
        bFinded = true;
        boost::property_tree::ptree& child = value.second;
